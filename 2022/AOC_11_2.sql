...
CREATE TABLE monkey(
  ...
  operation_operand bigint NULL, -- `NULL` means `old`
  test_divisible_by bigint NOT NULL,
...
CREATE TABLE item(
  ...
  worry_level bigint NOT NULL
);

CREATE INDEX item_index ON item(round, turn, monkey_id, ordering);

CREATE FUNCTION operate(a bigint, operator operator, b bigint)
  RETURNS bigint
  LANGUAGE SQL
  RETURN
    CASE operator
...
CREATE FUNCTION throw_items(round int, turn int)
  ...
  AS $$
  WITH finite_field(finite_field) AS (
    SELECT product(test_divisible_by) :: bigint FROM monkey
  )
  SELECT
    id AS source_monkey_id,
    CASE
      WHEN operate(worry_level, operation_operator, operation_operand) % test_divisible_by = 0
    ...
    operate(worry_level, operation_operator, operation_operand) % finite_field AS worry_level
  FROM monkey
  JOIN item
    ON round = $1 AND turn = $2
      AND monkey.id = item.monkey_id
  JOIN finite_field ON true
  ORDER BY ordering
  $$
  LANGUAGE SQL;
...
CREATE FUNCTION iterate(rounds int)
  ...
  BEGIN
    ...
    FOR current_round IN 1 .. rounds LOOP
      FOR current_turn IN 0 .. max_id LOOP
        INSERT INTO item (SELECT * FROM next_turn(current_round, current_turn, max_id));
      END LOOP;
    END LOOP;
  END
  $$
...
INSERT INTO item(
  ...
  FROM
    grouped,
    LATERAL string_to_table(regexp_substr(lines[2], '^ +Starting items: (.+)', 1, 1, '', 1), ', ') WITH ORDINALITY AS worry_level
  ORDER BY regexp_substr(lines[1], '^Monkey ([0-9]+):$', 1, 1, '', 1) :: int
);

SELECT iterate(10000);
