...
CREATE MATERIALIZED VIEW overlapped AS
  SELECT DISTINCT a.pair AS pair
    FROM assignment AS a
    JOIN assignment AS b
          ON a.pair = b.pair
          AND a.elf <> b.elf
   WHERE (a.range_start BETWEEN b.range_start AND b.range_end)
      OR (a.range_end BETWEEN b.range_start AND b.range_end);

\echo
\t on
\o

SELECT COUNT(*) FROM overlapped;
