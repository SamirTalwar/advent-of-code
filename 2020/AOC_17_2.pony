...
class val Coordinates is (Equatable[Coordinates] & collections.Hashable)
  ...
  new val create(w': ISize, x': ISize, y': ISize, z': ISize) =>
    w = w'
    x = x'
    y = y'
    z = z'
  ...
  fun eq(that: box->Coordinates): Bool =>
    (this.w == that.w) and (this.x == that.x) and (this.y == that.y) and (this.z == that.z)

  fun hash(): USize =>
    var result: USize = 17
    result = (result * 31) + w.hash()
...
class Neighbors is Iterator[Coordinates]
  ...
  fun ref next(): Coordinates ? =>
    if _state >= (3 * 3 * 3 * 3) then
      error
    end

    let w = (_state % 3) - 1
    let x = ((_state / 3) % 3) - 1
    let y = (((_state / 3) / 3) % 3) - 1
    let z = ((((_state / 3) / 3) / 3) % 3) - 1
    _state = _state + 1
    if _state == ((3 * 3 * 3 * 3) / 2) then
      _state = _state + 1
    end
    Coordinates(_coordinates.w + w, _coordinates.x + x, _coordinates.y + y, _coordinates.z + z)
...
actor Solution is Solve[Grid]
  ...
  be apply(grid: Grid) =>
    ...
    for (x, row) in grid.pairs() do
      for (w, cell) in row.pairs() do
        if cell is Active then
          active_cells = (active_cells(Coordinates(w.isize(), x.isize(), 0, 0)) = cell)
        end
      end
    end
