...
main = do
  ...
  let validItemCombinations = filter (\items -> (me `with` items) `beats` boss) itemCombinations
  let best = head $ List.sortBy (Ord.comparing (sum . map itemCost)) validItemCombinations
  print best
  print $ sum $ map itemCost best
