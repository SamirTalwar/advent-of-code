...
instance Ord Battle where
  compare a b =
    Ord.comparing (^. battleManaSpent) a b
...
play spell = spendMana spell . thump . applyEffects noSpell . applyEffects spell

applyEffects :: Spell -> Battle -> Battle
applyEffects newSpell (Battle wizard enemy effects manaSpent history) =
  let oneTimeAffectedWizard = newSpell ^. spellSelfOneTimeEffect $ wizard
