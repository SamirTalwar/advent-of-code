    ...
    for record in records {
        ...
        case .wakesUp:
            ...
            while time != record.timestamp.time {
                let guardAtTime = GuardAtTime(guardId: currentGuardId, time: time)
                asleep[guardAtTime] = (asleep[guardAtTime] ?? 0) + 1
                time = time + 1
            }
